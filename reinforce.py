...
class DataTrainingArguments:
    ...
    step_per_update: int = field(default=8, metadata={"help": "step per update"})
    teacher_ratio: float = field(default=0.8, metadata={"help": "teacher ratio"})
    method: str = field(default='None', metadata={"help": "training method"})
    use_kl: bool = field(default=False, metadata={"help": "use kl divergence"})
    kl_coef: float = field(default=1, metadata={"help": "kl divergence coefficient"})
    teacher_path: Optional[str] = field(default=None, metadata={"help": "teacher model path"})
    top_k : int = field(default=1,metadata={"help": "sampling top_k in REINFORCE"})
    do_sample : bool = field(default=False,metadata={"help": "sampling in REINFORCE"})
...
def reward_fn(reward_model, input_ids, output_ids, args, pad_token_id=None, n_step = 1, logits_processor=None, tokenizer=None):
    with torch.no_grad():
        ...
        with torch.no_grad():
            # Given that you are using the reward model to calculate the reward of the student-generated outputs, 
            # it's important to ensure that the logits used for reward calculation are consistent
            # and not influenced by the presence of labels during the forward pass.
...
def main():
    ...
    teacher = AutoModelForSeq2SeqLM.from_pretrained(data_args.teacher_path, torch_dtype=torch.float16)
    ...
    if data_args.use_kl==True:
        init_model = AutoModelForSeq2SeqLM.from_pretrained(
            model_args.model_name_or_path,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
            config=config,
            cache_dir=model_args.cache_dir,
    ...
    model = Net(seq2seq_model, teacher=teacher, args=training_args, 
                ...
                logits_processor=logits_proc, tokenizer=tokenizer,
                init_model=init_model, gen_kwargs=gen_kwargs)
    ...
    # )
    data_collator = DataCollatorWithIndices(
        tokenizer=tokenizer,
        model=model,
        label_pad_token_id=label_pad_token_id,
        pad_to_multiple_of=8 if accelerator.use_fp16 else None,
    ...
    for epoch in range(starting_epoch, training_args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            output_ids= model.math_generate(
                                batch["input_ids"],
                                attention_mask=batch["attention_mask"],
            ...
            # teacher.eval()
            # output_ids= teacher.generate(input_ids, 
            ...
            if data_args.n_step==1:
                ...
                returns = n_step_reward_shift_upgrade(rewards, 
                                                      selection_value, 
                                                      next_state_value, 
                                                      n_step=data_args.n_step)
            model.train()
            ...
            loss = model.reinforce(
                ...
                use_kl=data_args.use_kl,
                kl_coef=data_args.kl_coef,
            )
