...
def process(nprocs, inq, outq, tokenizer, seed):
    set_seed(seed)
    batch = []
    while True:
...
class DatasetForSeq2Seq:
    def __init__(self, dir, split, suffixes, max_length, ratio, tokenizer, num_workers, seed) -> None:
        super().__init__()
        ...
        processes = [ctx.Process(target=process, args=(
            num_workers, input_q[i], output_q, tokenizer, self.seed+i)) for i in range(num_workers)]
        for p in processes:
            p.start()
        ...
        while idx < total:
            for _pos, len_s, len_t in output_q.get():
                if 0 < len_s < self.max_length \
                        and 0 < len_t < self.max_length and 1/self.ratio <= len_s / len_t <= self.ratio:
                    pos.append(_pos)
...
class BatchedDataset:
    def __init__(self, dataset, max_tokens, tokenizer, num_workers,seed):
        ctx = mp.get_context('fork')
        ...
        self.q = ctx.Queue()
        self.batches = self._bachify(seed)
        self.seed = seed
        # set_seed(seed)
    ...
    def _bachify(self,seed):
        ...
        splits[-1] = length
        ctx = mp.get_context('fork')
        processes = [ctx.Process(target=self._batchify_proc, args=(self.max_tokens, splits[i], splits[i+1], self.dataset, seed+i)) for i in range(self.num_workers)]
        for p in processes:
            p.start()
    ...
    def _batchify_proc(self, max_tokens, start, end, dataset, seed):
        set_seed(seed)  # Ensure unique seed for each worker
        batches = []
        cur = start
        # start_cur = cur  # Initialize start_cur here to ensure it's always defined
        fp0 = open(dataset.f0, 'rb', buffering=0)
        fp1 = open(dataset.f1, 'rb', buffering=0)
        while cur < end:
            current_len = 0
            start_cur = cur # Update start_cur within the loop
            while cur < end:
                ...
                current_len += diff
                cur += 1
        if start_cur < end: # Check if there's any remaining data
...
class DataCollector:
    ...
    def __iter__(self):
        ...
        self.processes = [ctx.Process(target=self.fill_queue, args=(i,  i + self.rank * self.num_workers, self.dataset, self.seed+i)) for i in range(self.num_workers)]
        for p in self.processes:
            p.start()
        return self

    def fill_queue(self, pid, offset, dataset,seed):
        set_seed(seed) # Ensure unique seed for each worker
...
if __name__ == "__main__":
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        '/mnt/nvme/guoqing/t5-base')
    print('B1')
    d = DatasetForSeq2Seq('data/wmt_en-ro_100k',
                          'train', ('src', 'src'), max_length=256, ratio=1.5, tokenizer=tokenizer, num_workers=2, seed=0)
    
    dataset = BatchedDataset(d, 512, tokenizer, 1, seed=2)

    print('B2')
    c1 = DataCollector(dataset, num_workers=32, rank=0, world_size=1, shuffle=True, seed=1)
    # c2 = DataCollector(dataset, num_workers=1, rank=1, world_size=1, shuffle=True, seed=1)
